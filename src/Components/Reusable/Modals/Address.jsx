...
export const Address = () => {
  const { label, name, address, country, pincode, city, state, selectAddress } =
    useAddress();
    ...
    (async () => {
      try {
        ...
        const response = await axios.get(
          `${api.URL}${api.address.GETBYID}${userId}`
        );
          ...
          <div className="flex flex-col w-full flex-wrap">
            <div className="flex flex-row w-full flex-wrap mb-5">
              <div className="relative w-full border-none">
                <select
                  className="bg-gray-600 text-white appearance-none border-none inline-block py-3 pl-3 pr-8 rounded leading-tight w-full"
                  onChange={(e) => {
                    selectAddress(e);
                  }}
                >
                  <option className="pt-6" value="0">
                    {languageState.data.select_address_from_the_list ||
            ...
            <div className="flex flex-row w-full flex-wrap">
              <div className="flex flex-col w-full">
                <label className="px-2">
                  {languageState.data.address_type || "Address Type"}
                </label>
                <input
                  type="text"
                  placeholder="HOME"
                  defaultValue={label}
                  className="lg:w-full w-72 p-5 m-2"
                  disabled
                />
              </div>
            </div>
            <div className="flex flex-row w-full flex-wrap">
              ...
              <div className="flex flex-col">
                <label className="px-2">
                  {" "}
                  {languageState.data.address || "Address"}{" "}
                </label>
                <input
                  type="text"
            ...
            <div className="flex flex-row w-full flex-wrap">
              <div className="flex flex-col">
                <label className="px-2">
                  {languageState.data.country || "Country"}{" "}
                </label>
                <input
                  type="text"
                  placeholder="INDIA"
              ...
              <div className="flex flex-col">
                <label className="px-2">
                  {languageState.data.pincode || "Pincode"}
                </label>
                <input
                  type="text"
                  placeholder="831011"
            ...
            <div className="flex flex-row w-full flex-wrap">
              ...
              <div className="flex flex-col">
                <label className="px-2">
                  {languageState.data.state || "State"}
                </label>
                <input
                  type="text"
                  placeholder="JHARKAHAND"
                  defaultValue={state}
                  className="w-72 p-5 m-2"
                  disabled
                />
